...
export default function DashboardPage() {
  ...
  const reload = useCallback(() => {
    fetchAppointments(date).then(setAppointments).catch(() => {});
  }, [date]);
      ...
      <main className="flex flex-1 flex-col gap-4 overflow-hidden p-6">
        ...
        <div className="flex flex-1 gap-4 overflow-hidden">
          ...
          <div className="flex w-72 shrink-0 flex-col gap-4 overflow-y-auto">
                  ...
                  <li key={a.id} className="rounded-lg border border-gray-100 p-2 text-xs">
                    ...
                    <div className="mt-1 flex items-center justify-between">
                      ...
                      {a.status !== "cancelled" && a.status !== "completed" && (
                        <button
                          onClick={() => updateAppointmentStatus(a.id, "cancelled").then(reload)}
            ...
            <QuickAppointmentForm services={services} staff={staff} date={date} onCreated={reload} />
          </div>
        </div>
      </main>
